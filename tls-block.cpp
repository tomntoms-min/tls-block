...
#pragma pack(push, 1)
struct PseudoHdr {
    uint32_t src_ip; uint32_t dst_ip; uint8_t reserved{0}; uint8_t protocol; uint16_t tcp_len;
};
#pragma pack(pop)
...
static void send_forward_rst(pcap_t* handle, const ReassemblyContext& ctx, const Mac& my_mac) {
    ...
    ip->total_length = htons(sizeof(IpHdr) + sizeof(TcpHdr)); ip->checksum = 0;
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (res == 0) continue; if (res < 0) break;
        const EthHdr* eth = (const EthHdr*)packet; if (eth->type() != EthHdr::Ip4) continue;
        const IpHdr* ip = (const IpHdr*)(eth + 1); if (ip->protocol != IpHdr::TCP) continue;
