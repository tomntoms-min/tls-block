...
void PacketHandler::handlePacket(const struct pcap_pkthdr* header, const uint8_t* packet) {
    ...
    if (tls_hdr->content_type_ != TlsHdr::Handshake) {
        active_streams_.clear(); // Not a handshake, clear streams
        return;
    }

    const HandshakeHdr* handshake_hdr = reinterpret_cast<const HandshakeHdr*>(payload + sizeof(TlsHdr));
    if (handshake_hdr->handshake_type_ != 0x01) { // Client Hello
        return;
    }
    ...
    if (payload_len < expected_tls_len) { // Segmented packet
        Stream& stream = active_streams_[flow];
        if (stream.data.empty()) { // First segment
...
bool PacketHandler::findSNIAndBlock(const Flow& flow, const uint8_t* tls_data, size_t tls_len) {
    // Basic SNI parsing logic (simplified)
    // tls_data = start of TLS Record
    size_t offset = sizeof(TlsHdr) + sizeof(HandshakeHdr) + 38; // To session ID length
    ...
    while (&tls_data[offset] + 4 <= extensions_end) {
        ...
        if (ext_type == 0x0000) { // server_name extension
            ...
            // SNI list length (2 bytes), name type (1 byte), name length (2 bytes)
            if (ext_len < 5) break; 
            ...
            if (server_name == target_server_name_) {
                ...
                if (stream_it != active_streams_.end()) { // Segmented
                    ...
                    IpHdr ip_hdr;
                    TcpHdr tcp_hdr;
                    // Reconstruct headers from flow (simplified for this example)
                    sendForwardRst(eth_hdr, ip_hdr, tcp_hdr, tls_len); // Note: Headers might not be perfect here
...
void PacketHandler::sendBackwardRst(const IpHdr& ip_hdr_orig, const TcpHdr& tcp_hdr_orig, size_t payload_len) {
    ...
    tcp_hdr->th_flags = TcpHdr::RST | TcpHdr::ACK;
    tcp_hdr->th_seq = tcp_hdr_orig.th_ack; // Backward SEQ is original ACK
